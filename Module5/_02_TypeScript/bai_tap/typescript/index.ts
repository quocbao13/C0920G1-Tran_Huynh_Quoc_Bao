...
async function fetchRepo(nameSearch : string): Promise<Array<ISingleRepo>> {

    let res: Response | IRepos = await fetch('https://api.github.com/search/repositories?q='+nameSearch);
...
async function main() {
    container.innerHTML = "";
    // step 1: fetch repo
    let name =(<HTMLInputElement>document.getElementById("nameSearch"));
    let nameSearch = name.value;
    const res = await fetchRepo(nameSearch);
